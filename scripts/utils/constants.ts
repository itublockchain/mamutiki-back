export const DEFAULT_VALUES = {
  campaign: {
    title: "This is a test campaign",
    description:
      "This is a test description, it can be longer, we love movement, mammmothon.",
    prompt:
      "This is a test prompt, it can be longer, we love movement, mammmothon.",
    unitPrice: 0.0001, // DATA
    minContribution: 0, // DATA
    minScore: 1,
    rewardPool: 0.1, // DATA
    publicKeyForEncryption:
      "",
  },
  contribution: {
    dataCount: 1,
    storeCid: "test",
    score: 100,
    keyForDecryption: "test",
  },
  subscription: {
    price: 250, // DATA
  },
};

export const ONE_TOKEN = 1000000;
